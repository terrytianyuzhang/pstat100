...
outlier_indices <- match(rownames(outliers), rownames(x_ctr))
...
outlier_plot_df <- outlier_data %>%
  pivot_longer(cols = -County, names_to = "Variable", values_to = "Standardized_value")

# Create shaded area within 3SD of mean
grey_shade <- data.frame(
  Variable = colnames(x_ctr),
  upr = rep(3, ncol(x_ctr)),
  lwr = rep(-3, ncol(x_ctr))
)
...
# Create base plot
ticks_plot <- ggplot(outlier_plot_df, aes(x = Standardized_value, y = Variable)) +
  
  # Add tick marks for outlier values
  geom_point(shape = "|", size = 3) +
  
  # Add shaded region for ±3 SD range
  geom_rect(data = grey_shade, aes(xmin = lwr, xmax = upr, ymin = Variable, ymax = Variable), 
            fill = "gray", alpha = 0.2, inherit.aes = FALSE) +
  
  # Add mean markers
  geom_point(data = means_df, aes(x = group_mean, y = Variable, color = large), size = 3) +
  
  # Axis labels and minimal theme
  labs(x = "Standardized Value", y = "Variable", title = "Outlier Standardized Values") +
  theme_minimal() +
